...
This program is a simple Unix shell called wish.The shell can be invoked with either no arguments or a single argument; The output can be redirecte to a file named by user; It supports some shell's basic function, like find the path of "ls" or "cd", and also supports parallel command line or reading from a file.
