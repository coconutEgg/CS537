...
function sort_run {
	...
	if ! gcc -o sort_m${1}r${2} sort.c mapreduce.c -g -pthread -O -lm -DNUM_MAPPERS=${1} -DNUM_REDUCERS=${2} -DFILE_OUTPUT_SUFFIX="\"f${3}m${1}r${2}_t${test_cnt}\""; then
...
function wordcount_run {
	...
	if ! gcc -o wordcount_m${1}r${2} wordcount.c mapreduce.c -g -pthread -O -lm -DNUM_MAPPERS=${1} -DNUM_REDUCERS=${2} -DFILE_OUTPUT_SUFFIX="\"f${3}m${1}r${2}_t${test_cnt}\""; then
		echo "[Test${test_cnt} failed]"
...
function wordcount_cp_run {
	...
	if ! gcc -o wordcount_cp_m${1}r${2} wordcount_cp.c mapreduce.c -g -pthread -O -lm -DNUM_MAPPERS=${1} -DNUM_REDUCERS=${2} -DFILE_OUTPUT_SUFFIX="\"f${3}m${1}r${2}_t${test_cnt}\""; then
		echo "[Test${test_cnt} failed]"
...
function wordcount_multi_run {
	...
	if ! gcc -o wordcount_multi_m${1}r${2} wordcount_multi.c mapreduce.c -g -pthread -O -lm -DNUM_MAPPERS=${1} -DNUM_REDUCERS=${2} -DNUM_ITERATION=${6} -DFILE_OUTPUT_SUFFIX="\"f${3}m${1}r${2}_t${test_cnt}\""; then
		echo "[Test${test_cnt} failed]"
...
function wordcount_weight_run {
	...
	if ! gcc -o wordcount_weight_m${1}r${2} wordcount_weight.c mapreduce.c -g -pthread -O -lm -DNUM_MAPPERS=${1} -DNUM_REDUCERS=${2} -DFILE_OUTPUT_SUFFIX="\"f${3}m${1}r${2}_t${test_cnt}\""; then
		echo "[Test${test_cnt} failed]"
    	echo "  Failed to build wordcount_m${1}r${2}"
    	echo
    	if [ ${continue_flag} -eq 1 ]; then
